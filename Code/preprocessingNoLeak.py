...
#        test['Named'] = (test['NameLength']).astype(int)
    ...
    if preprocessingFlags['Seasons']:
        train['Season']=train['Month'].apply(getSeason)
        test['Season']=test['Month'].apply(getSeason)
    ...
    if preprocessingFlags['Holidays']:
        train['Holidays'] = train['DateTime'].apply(lambda x: isHoliday(x.month, x.day))
        test['Holidays'] = test['DateTime'].apply(lambda x: isHoliday(x.month,x.day))
